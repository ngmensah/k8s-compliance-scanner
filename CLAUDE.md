...
This is the pinned boot file. It loads automatically at the start of every session. It survives context compaction; VAULT-INDEX.md may not, so the rules that can't lapse live here. The full operating manual is VAULT-INDEX.md at the vault root — read it at startup.

## Where this file goes, and where your vault is

This file lives in the folder you run Claude Code from (your "working directory"), separate from your notes. Your vault (the notes) lives at:

```
/home/ngmensah/vault        ← CLI (local machine)
~/vault                     ← Browser (claude.ai/code cloud session)
```

The vault is also a private GitHub repo at `https://github.com/ngmensah/vault`. All sessions read from and write back to this repo so progress stays in sync everywhere.

## Startup Sequence

**CLI session (local machine):**
1. Run `git -C /home/ngmensah/vault pull` to get the latest vault from GitHub.
2. Read `VAULT-INDEX.md` at the vault root — the profile, the rules, the system map.
3. Check yesterday's daily note in `01 - Daily Notes/`; if you have context it's missing, backfill it.
4. Scan `Active Priorities.md` for what's currently open, so nothing queued slips.

**Browser session (claude.ai/code):**
1. Ask the user to run: `git clone https://GITHUB_TOKEN@github.com/ngmensah/vault.git ~/vault` — token is saved in Bitwarden as "GitHub Vault Token". Wait for them to confirm it's done.
2. Read `VAULT-INDEX.md` at `~/vault/VAULT-INDEX.md`.
3. Check yesterday's daily note in `~/vault/01 - Daily Notes/`.
4. Scan `~/vault/Active Priorities.md` for what's currently open.
...
- **Checkpoint persistence.** Any time something changes that a future session would need to know, persist it without being asked: update the relevant vault note, today's daily note, and this file (only for a new always-on rule). Then scan the touched folder's index and cross-referenced notes for drift and fix them in the same pass. Verify each change landed by reading it back. When in doubt, save. After every vault write, commit and push: `git -C /home/ngmensah/vault add -A && git -C /home/ngmensah/vault commit -m "vault update [date]" && git -C /home/ngmensah/vault push` (use `~/vault` path in browser sessions).
...
- **Renaming notes.** A rename done outside the app (e.g. a shell `mv`) breaks the `[[links]]` that point to the note. Obsidian only auto-repairs them when you rename **inside the Obsidian app**. So do renames in the app; if the AI must rename a file directly, it then has to find and fix every `[[old name]]` reference by hand.
- **Daily notes.** Live in `01 - Daily Notes/`, in monthly subfolders named `NN - Month YYYY` (e.g. `07 - July 2026`), filename `YYYY-MM-DD.md`. One note per day; if today's already exists, append a new `## Session N` rather than overwriting.

## Make it yours

No additional rules yet beyond the defaults above and in VAULT-INDEX.md — will add here as they come up.